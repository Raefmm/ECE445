...
#define LED1_BUILTIN 39 // 32
#define LED2_BUILTIN 37 // 30
#define LED3_BUILTIN 38 // 31
#define LEDG_BUILTIN 36 // 29
...
void CheckLEDs(){
  ...
  else leds[3] = !leds[0] & !leds[1] & !leds[2];
}
...
bool safeRead(std::string & name, bool equal, std::string old = "x"){
  ...
  //    name = pCharacteristic_2->getValue();
  //  
  //    if(condition(name, old))  return true;
  //  }
  //
  //  return false;
...
void setup() {
  ...
  pinMode(RFID_EN_PIN, OUTPUT);
  pinMode(LightSensor_EN_PIN, OUTPUT);

  digitalWrite(RFID_EN_PIN, LOW);
  digitalWrite(LightSensor_EN_PIN, LOW);

  digitalWrite(LED1_BUILTIN, HIGH); //active low
  digitalWrite(LED2_BUILTIN, HIGH);
  digitalWrite(LED3_BUILTIN, HIGH);
  digitalWrite(LEDG_BUILTIN, HIGH);
  ...
  for(int i = 0; i < NUMBER_OF_SENSORS; ++i){
    ...
    if(i == 2)  myMuxRFID.setPort(7);
    else  myMuxRFID.setPort(i);
    if(! myRfids[i]->begin())
  ...
  for(int i = 0; i < NUMBER_OF_SENSORS; ++i){
    ...
    if(i == 2) myMuxLight.setPort(7);
    else  myMuxLight.setPort(i);
    if ( ! ltr[i].begin() ) {
...
void loop() {
  ...
  for(int i = 0; i < NUMBER_OF_SENSORS; ++i){
    if(visible_plus_ir[i] > LightThreshold){
      ...
      if(timerReadSeconds(Timer0_Cfg) > 30) {
        break;
      }

      timerStart(Timer0_Cfg);

      if(leds[0]) digitalWrite(LED1_BUILTIN, LOW); //active low
      if(leds[1]) digitalWrite(LED2_BUILTIN, LOW);
      if(leds[2]) digitalWrite(LED3_BUILTIN, LOW);
      if(leds[3]) digitalWrite(LEDG_BUILTIN, LOW);
      break;
    }
  }
  ...
  digitalWrite(LED1_BUILTIN, HIGH); //HIGH active low
  digitalWrite(LED2_BUILTIN, HIGH);
  digitalWrite(LED3_BUILTIN, HIGH);
  digitalWrite(LEDG_BUILTIN, HIGH);


  if( (visible_plus_ir[0] <= LightThreshold) && (visible_plus_ir[1] <= LightThreshold) && (visible_plus_ir[2] <= LightThreshold)){
    timerWrite(Timer0_Cfg, 0);
    timerStop(Timer0_Cfg);
  }

  if( visible_plus_ir[0] <= LightThreshold ) digitalWrite(LED1_BUILTIN, HIGH);
  if( visible_plus_ir[1] <= LightThreshold ) digitalWrite(LED2_BUILTIN, HIGH);
  if( visible_plus_ir[2] <= LightThreshold ) digitalWrite(LED3_BUILTIN, HIGH);
  if( visible_plus_ir[2] <= LightThreshold ) digitalWrite(LEDG_BUILTIN, HIGH);
